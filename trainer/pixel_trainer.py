...
class TrainerModulePixel:
    ...
    def __init__(
        ...
        self.model = ScoreNet(marginal_prob_std_fn)

        # Batch from evaluation for shape initialization
        self.fake_input = next(iter(train_loader))
        self.fake_time = jnp.ones(self.fake_input[0].shape[0])
    ...
    def init_model(self):
        ...
        model_params = self.model.init({'params': rng}, self.fake_input[0].permute(0, 2, 3, 1), self.fake_time)
