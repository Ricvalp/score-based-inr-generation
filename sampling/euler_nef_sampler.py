...
def Euler_Maruyama_nef_sampler(
    rng,
    score_model,
    params,
    marginal_prob_std,
    ...
    @jax.jit
    def score_fn(x, t, rng):
        return score_model.apply(params, x, t, rngs=rng)

    rng, step_rng, dropout_rng = jax.random.split(rng, 3)
    time_shape = (batch_size,)
    sample_shape = time_shape + nef_shape
    init_x = jax.random.normal(step_rng, sample_shape) * marginal_prob_std(1.0)
    ...
    for time_step in tqdm(time_steps):
        ...
        mean_x = x + (g**2) * score_fn(x, batch_time_step, {"dropout": dropout_rng}) * step_size
