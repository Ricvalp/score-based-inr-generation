...
class ScoreNet1D(nn.Module):
    ...
    def __call__(self, x, t):
        ...
        h = nn.Dense(self.channels[1])(jnp.concatenate([h, h3], axis=-1))
        h += Dense(self.channels[1])(embed)
        h = nn.GroupNorm()(h)
        h = act(h)

        h = nn.Dense(self.channels[0])(jnp.concatenate([h, h2], axis=-1))
        h += Dense(self.channels[0])(embed)
        h = nn.GroupNorm()(h)
        h = act(h)
        h = nn.Dense(x.shape[1])(jnp.concatenate([h, h1], axis=-1))
